...
void main() {
  ...
  void parseType(String nodeType, String name) {
    ...
    if (category == "enum") {
      XmlElement? enumNode = document.findAllElements("enums").where((element) =>
          element.getAttribute("name") == (name) &&
          element.getAttribute("api") != "vulkansc").firstOrNull;
      if (enumNode == null) {
        enumNode = document.findAllElements("enums").where((element) =>
        element.getAttribute("name") == name.replaceAll("KHR", "") &&
            element.getAttribute("api") != "vulkansc").firstOrNull;
      }
  ...
  extensions.forEach((name) {
    ...
    String? extension_name = extension.getAttribute("name")!;
    ...
    requirements.forEach((element) {
      element.childElements.forEach((node) {
        ...
        if (nodeType == "command") {
          XmlElement? VkNode = document.findAllElements(nodeType).where(
              (element) =>
                  element.getElement("proto")?.getElement("name")?.innerText == extension_name).firstOrNull;

          if (VkNode == null) {
            VkNode = document.findAllElements(nodeType).where(
                    (element) =>
                element.getElement("proto")?.getElement("name")?.innerText == extension_name?.replaceAll("KHR", "")).firstOrNull;
            if (VkNode == null) return;
          }
